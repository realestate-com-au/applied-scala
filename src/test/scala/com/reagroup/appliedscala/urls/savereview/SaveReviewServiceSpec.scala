...
class SaveReviewServiceSpec extends Specification {
  val movie = Movie(
    "Empire Strikes Back",
    "Guy unwittingly is kissed by sister",
    Vector.empty
  )
  ...
  "saveReview" should {
    ...
    "return errors for invalid review" in {
      ...
      val saveReview =
        (movieId: MovieId, review: ValidatedReview) => IO.pure(ReviewId(12345))
      ...
      result.unsafeRunSync() must_=== NonEmptyList
    ...
    "return errors for non-existent movie" in {
      ...
      result.unsafeRunSync() must_=== NonEmptyList
        .of(MovieDoesNotExist, ReviewAuthorTooShort, ReviewCommentTooShort)
        .invalid
