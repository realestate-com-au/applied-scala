...
import com.reagroup.appliedscala.urls.repositories.{
  Http4sMetascoreRepository,
  PostgresqlRepository
}
import com.reagroup.appliedscala.urls.fetchallmovies.{
  FetchAllMoviesController,
  FetchAllMoviesService
}
import com.reagroup.appliedscala.urls.fetchenrichedmovie.{
  FetchEnrichedMovieController,
  FetchEnrichedMovieService
}
import com.reagroup.appliedscala.urls.fetchmovie.{
  FetchMovieController,
  FetchMovieService
}
import com.reagroup.appliedscala.urls.savemovie.{
  SaveMovieController,
  SaveMovieService
}
import com.reagroup.appliedscala.urls.savereview.{
...
class AppRuntime(
  ...
  /** This is the repository that talks to Postgresql
    */
  private val pgsqlRepo =
    PostgresqlRepository(config.databaseConfig, contextShift)

  private val http4sMetascoreRepo =
    new Http4sMetascoreRepository(httpClient, config.omdbApiKey)
  ...
  private val fetchAllMoviesController: FetchAllMoviesController = {
    val fetchAllMoviesService: FetchAllMoviesService =
      new FetchAllMoviesService(pgsqlRepo.fetchAllMovies)
  ...
  private val fetchMovieController: FetchMovieController = {
    new FetchMovieController(_ =>
      ???
    ) // Construct a `FetchMovieService`, then fill this out
  }

  private val fetchEnrichedMovieController: FetchEnrichedMovieController = {
    val fetchEnrichedMovieService = new FetchEnrichedMovieService(
      pgsqlRepo.fetchMovie,
      http4sMetascoreRepo.apply
    )
  ...
  private val saveReviewController: SaveReviewController = {
    val saveReviewService =
      new SaveReviewService(pgsqlRepo.saveReview, pgsqlRepo.fetchMovie)
  ...
  private val appRoutes = new AppRoutes(
    ...
    fetchMovieHandler = _ =>
      IO(
        Response[IO](status = Status.NotImplemented)
      ), // Fill this out after constructing `FetchMovieController`
   ...
   */
  def routes: HttpApp[IO] = HttpApp((req: Request[IO]) =>
    appRoutes.openRoutes(req).getOrElse(Response[IO](status = Status.NotFound))
  )
